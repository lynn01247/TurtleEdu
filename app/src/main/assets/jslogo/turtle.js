...
//          });
        preImage("./turtlepng2.png",function(){
...
//        this.drawAxis();
    ...
    /* 绘制网格 */
    drawGrid: {value: function(color,stepx,stepy) {
      var ctx = this.turtle_ctx;
      ctx.strokeStyle = color;
      ctx.lineWidth = 0.5;
      for(var i=stepx + 0.5; i<canvas.width/2; i += stepx){
        ctx.beginPath();
      ...
      for(var i=stepy + 0.5; i<canvas.height/2; i += stepy) {
        ctx.beginPath();
    ...
    drawAxis: {value: function() {
      ...
      this.drawVerticalAxisTicks(ctx);
      this.drawHorizontalAxisTicks(ctx);
      ctx.restore();
    }},
    /* 绘制水平轴线 */
    drawHorizontalAxis: {value: function(ctx) {
      var yy;
      if(AXIS_TOP>1000){
        yy = 300;
      }else{
      ...
      ctx.lineTo(yy, AXIS_ORIGIN.y);//AXIS_RIGHT
      ctx.stroke();
      ctx.moveTo(AXIS_ORIGIN.x, AXIS_ORIGIN.y);
      ctx.lineTo(-yy, AXIS_ORIGIN.y);//AXIS_LEFT
      ctx.stroke();
    }},
    /* 绘制垂直轴线 */
    drawVerticalAxis: {value: function(ctx) {
      var yy;
      if(AXIS_TOP>1000){
        yy = 300;
      }else{
      ...
      ctx.beginPath();
      ctx.moveTo(AXIS_ORIGIN.x, AXIS_ORIGIN.y);
      ctx.lineTo(AXIS_ORIGIN.x, yy);//AXIS_TOP
      ctx.stroke();
      ctx.moveTo(AXIS_ORIGIN.x, AXIS_ORIGIN.y);
      ctx.lineTo(AXIS_ORIGIN.x, -yy);//AXIS_BOTTOM
      ctx.stroke();
    }},
    /* 绘制垂直轴线刻度 */
    drawVerticalAxisTicks: {value: function(ctx) {
      var deltaY;
      var textPaint = false;
      for(var i=1; i<NUM_VERTICAL_TICKS; i++){
        //console.log('&&&&' + NUM_VERTICAL_TICKS);
        if(i*VERTICAL_TICK_SPACING > 150){
          break;
        }
        ctx.beginPath();
        if(i%5===0){
        ...
        if(textPaint){
          ctx.scale(1,-1);
          ctx.fillText(Number(i*VERTICAL_TICK_SPACING), deltaY+5, AXIS_ORIGIN.y-i*VERTICAL_TICK_SPACING+4);
          ctx.stroke();
          ctx.scale(1,-1);
        }
        ctx.moveTo(AXIS_ORIGIN.x-deltaY, i*VERTICAL_TICK_SPACING-AXIS_ORIGIN.y);
        ctx.lineTo(AXIS_ORIGIN.x+deltaY, i*VERTICAL_TICK_SPACING-AXIS_ORIGIN.y);
        ctx.stroke();
        if(textPaint){
          ctx.scale(1,-1);
          ctx.fillText(Number(-i*VERTICAL_TICK_SPACING), deltaY+5, i*VERTICAL_TICK_SPACING-AXIS_ORIGIN.y+4);
          ctx.stroke();
          ctx.scale(1,-1);
        }
      }
    ...
    /* 绘制水平轴线刻度 */
    drawHorizontalAxisTicks: {value: function(ctx) {
      var deltaX;
      var textPaint = false;
      for(var i=1; i<NUM_HORIZONTAL_TICKS; i++){
        ...
        if(textPaint){
          ctx.scale(1,-1);
          ctx.fillText(Number(i*VERTICAL_TICK_SPACING), AXIS_ORIGIN.x+i*HORIZONTAL_TICK_SPACING-5,AXIS_ORIGIN.y+deltaX+12);
          ctx.stroke();
          ctx.scale(1,-1);
        }
        ctx.moveTo(AXIS_ORIGIN.x-i*HORIZONTAL_TICK_SPACING,AXIS_ORIGIN.y-deltaX);
